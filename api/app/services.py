...
async def _maybe_llm_planner(
    ...
    try:
        prompt = _planner_prompt(request, customer, analyst, evidence, planner)
        if risk_review:
            prompt += f"\n\nRisk/Compliance Feedback:\n{risk_review.critique}\nConcerns: {', '.join(risk_review.concerns)}"
...
async def _maybe_llm_risk(customer: Customer, planner: PlannerOutput, risk_review: RiskReview) -> RiskReview:
    if not llm_is_enabled():
        return risk_review

    try:
        payload = await generate_json(_risk_prompt(customer, planner, risk_review))
        return RiskReview.model_validate(payload)
    except (LLMRequestError, ValidationError, ValueError):
        return risk_review


async def _maybe_llm_summary(customer: Customer, summary: str, arbiter: ArbiterDecision) -> str:
    if not llm_is_enabled():
        return summary

    try:
        payload = await generate_json(_comms_prompt(customer, summary, arbiter), max_tokens=min(settings.llm_max_tokens, 300))
...
def list_approvals(session: Session, page: int = 1, page_size: int = 10) -> PaginatedResponse[ApprovalRequest]:
    total = session.scalar(select(func.count(Approval.id))) or 0
    offset = (page - 1) * page_size
    approvals = list(session.scalars(select(Approval).order_by(desc(Approval.created_at)).offset(offset).limit(page_size)))
...
def list_audit_records(session: Session, page: int = 1, page_size: int = 25) -> PaginatedResponse[AuditRecordSchema]:
    total = session.scalar(select(func.count(AuditRecord.id))) or 0
    offset = (page - 1) * page_size
    records = list(session.scalars(select(AuditRecord).order_by(desc(AuditRecord.created_at)).offset(offset).limit(page_size)))
...
def stream_bounded_workflow(session: Session, request: WorkflowRequest):
...
def list_workflows(session: Session, customer_id: str | None = None) -> list[WorkflowRunSummary]:
    ...
    if customer_id:
        query = query.where(WorkflowRun.customer_id == customer_id)
    
    query = query.order_by(desc(WorkflowRun.submitted_at)).limit(20)
    runs = list(session.scalars(query))
    
    summaries = []
    for run in runs:
        # Get the recommendation from the decision record
        decision = session.scalar(select(RunDecision).where(RunDecision.run_id == run.id))
        summaries.append(
            WorkflowRunSummary(
                id=run.id,
                workflowType=run.workflow_type,
                ...
                status=run.status,
                summary=run.summary,
                finalRecommendation=decision.arbiter_final_recommendation if decision else "No recommendation available"
            )
        )
    return summaries
...
def run_bounded_workflow(session: Session, request: WorkflowRequest) -> WorkflowResponse:
    ...
    if llm_is_enabled():
        for _ in range(settings.llm_debate_iterations):
            planner = asyncio.run(_maybe_llm_planner(request, customer, analyst, evidence, planner, risk_review))
            risk_review = asyncio.run(_maybe_llm_risk(customer, planner, risk_review))
            if risk_review.verdict == "pass":
                break
